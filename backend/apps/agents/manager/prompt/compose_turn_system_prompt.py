...
def compose_turn_system_prompt(
    ...
    try:
        ...
        if load_builtin_permissions().get("ShowUI", "always_allow") != "deny":
            rich_ui_note = (
                "<rich_ui>\n"
                "Rich components are the DEFAULT way to present results; plain prose is the exception, "
                "for content with no structure and no media. The tools are "
                "mcp__openswarm-core__ShowUI and mcp__openswarm-core__AskUI; they are always available "
                "this session, so call them DIRECTLY by that name, no ToolSearch step needed.\n"
                "- Media ALWAYS renders: any image URL you surface (search results, fetched pages, "
                "APIs) goes through image or image-gallery, never a bare link or markdown; video = "
                "video, sound = audio. Remote URLs load straight from the internet.\n"
                ...
                "code = code-block, a diff = code-diff, a chart = chart, a map = geo-map, a source "
                "quote = citation, browsable items = item-carousel, command output = terminal, weather "
                "= weather, a draft to send = message-draft, a social post = x-post/linkedin-post/"
                "instagram-post, a receipt = order-summary. Render the component, then add one line of "
                "text.\n"
